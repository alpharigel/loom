...
Base image (`docker/Dockerfile`) built on node:20 with git, zsh, GitHub CLI, and Claude Code CLI. Containers mount workspace, gh config, SSH keys, and gitconfig. Environment variables (ANTHROPIC_API_KEY, OPENAI_API_KEY, GITHUB_TOKEN) are passed through.
